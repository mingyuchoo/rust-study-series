...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let vector_size: u64 = env::var("QDRANT_VECTOR_SIZE").ok().and_then(|v| v.parse::<u64>().ok()).unwrap_or(384);
    ...
    create_collection(&url, &collection_name_2, Some(vector_size), Some(Distance::Dot)).await?;
