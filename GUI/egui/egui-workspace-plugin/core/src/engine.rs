...
use plugin_interface::{FileFormat, ConversionOptions, ConversionResult as PluginConversionResult};
use std::path::Path;
use std::sync::Arc;
use log::{info, error, warn};

/// Conversion Engine handles file conversion operations using registered plugins
...
impl ConversionEngine {
    ...
    /// Convert a single file to the specified output format using the given plugin
    /// 
    /// # Arguments
    ...
    pub fn convert_file(
        ...
        info!("Starting conversion: {:?} -> {} using plugin '{}'", 
        ...
        if !input_path.exists() {
            ...
            return Err(ConversionError::InvalidInput(
                format!("파일이 존재하지 않습니다: {:?}", input_path)
            ));
        }
        ...
        if !input_path.is_file() {
            ...
            return Err(ConversionError::InvalidInput(
                format!("파일이 아닙니다: {:?}", input_path)
            ));
        }
        
        // 2. Get plugin from registry
        let plugin = self.registry.get_plugin(plugin_name)
            .ok_or_else(|| {
        ...
        if !plugin.can_convert(&input_format, output_format) {
            warn!("Plugin '{}' cannot convert {} -> {}", 
                  plugin_name, input_format.extension, output_format.extension);
            return Err(ConversionError::NoPluginAvailable(
                input_format.extension,
                output_format.extension.clone(),
            ));
        }
        
        // 5. Execute conversion
        info!("Executing conversion with plugin '{}'", plugin_name);
        let result = plugin.convert(input_path, output_format, options)
            .map_err(|e| {
    ...
    fn detect_format(&self, path: &Path) -> ConversionResult<FileFormat> {
        let extension = path.extension()
            ...
            .ok_or_else(|| {
                ConversionError::UnsupportedFormat(
                    format!("파일 확장자를 감지할 수 없습니다: {:?}", path)
                )
            })?;
        ...
        Err(ConversionError::UnsupportedFormat(
            format!("지원하지 않는 파일 형식: {}", extension)
        ))
    }
...
mod tests {
    ...
    impl Plugin for MockPlugin {
        ...
        fn can_convert(&self, from: &FileFormat, to: &FileFormat) -> bool {
            from.extension == "txt" && (to.extension == "txt" || to.extension == "md")
        }
        ...
        ) -> Result<PluginConversionResult, Box<dyn Error>> {
    ...
    fn test_batch_convert() {
        ...
        let results = engine.batch_convert(
            vec![file1.as_path(), file2.as_path()],
            &output_format,
            "mock-plugin",
            &options,
        );
