...
pub fn delete_post(conn: &mut PgConnection, pattern: &str) -> usize {
    ...
    diesel::delete(posts.filter(title.like(pattern))).execute(conn).expect("Error deleting posts")
}

pub fn update_post(conn: &mut PgConnection, id: i32) -> Post {
    use crate::schema::posts::dsl::*;
    diesel::update(posts.find(id)).set(published.eq(true)).get_result::<Post>(conn).unwrap()
}

pub fn show_posts(conn: &mut PgConnection) {
    use crate::schema::posts::dsl::*;
    let results = posts.filter(published.eq(true)).limit(5).load::<Post>(conn).expect("Error loading posts");
