...
use lib_index::{
    ...
    types::{ProcessedDocument, Chunk, ChunkKind, Embeddings3},
    RegexNer,
};
...
pub async fn index_create(
    ...
    if let Some(pdf_path) = payload.pdf_path.clone() {
        // a) PDF → 청킹
        let chunks_h = pdf_processor::process_pdf(&pdf_path)
            .map_err(|e| Error::External(format!("PDF 처리 실패: {}", e)))?;
        ...
        let chunk_embeddings: Vec<Embeddings3> = if payload.use_tfidf {
            embedding::embed_chunks_e3(&chunks_h, EmbeddingMode::Tfidf)
                .map_err(|e| Error::External(format!("TF-IDF 임베딩 실패: {}", e)))?
        } else {
            ...
            // 래핑
            chunks_h.iter().enumerate().map(|(i, ch)| Embeddings3 {
        ...
        let entity_embeddings: Vec<Embeddings3> = if payload.use_tfidf {
            embedding::embed_texts_e3(&entity_texts, EmbeddingMode::Tfidf)
                .map_err(|e| Error::External(format!("TF-IDF 엔티티 임베딩 실패: {}", e)))?
        } else {
        ...
        let relation_embeddings: Vec<Embeddings3> = if payload.use_tfidf {
            embedding::embed_texts_e3(&relation_texts, EmbeddingMode::Tfidf)
                .map_err(|e| Error::External(format!("TF-IDF 관계 임베딩 실패: {}", e)))?
        } else {
