...
use lib_index::{
    ...
    types::{ProcessedDocument, Embeddings3},
    RegexNer,
};
use lib_db::DB;
...
pub async fn reindex_pdfs(
        ...
        let entity_embeddings: Vec<Embeddings3> = if use_tfidf {
            ...
            let sems = match state.azure.embed(&refs).await {
                ...
                Err(e) => {
                    ...
                    item.error = Some(e.to_string()); results.push(item); continue;
                }
            };
    ...
    info!(
        ...
        results.len(), success, failed, elapsed
    );
