...
// =============================================================================
//
// 이 모듈은 런타임 심볼이 없다. `Makefile.toml` 의 데스크톱 릴리즈 태스크
// 존재를 컴파일/테스트 타임에 보장하기 위한 smoke test 전용 모듈이다.
//
...
//     추적성 체인(SPEC -> TC -> 구현)을 유지하기 위해 Rust 테스트 하네스에
//     묶는다.
//   - `Makefile.toml` 은 빌드 설정이라 런타임 심볼이 없고, 전통적인 함수 단위
//     단위 테스트를 붙일 수 없다. 문자열 smoke 검증으로 대체한다.
//
// 테스트 범위:
...
mod tests {
    ...
    fn read_root_makefile() -> String {
        ...
        let path: PathBuf = PathBuf::from(manifest_dir).join("..").join("..").join("Makefile.toml");
    ...
    fn test_tc_1_makefile_has_desktop_release_windows() {
        ...
        assert!(content.contains("\"tauri\""), "desktop release tasks must invoke cargo tauri");
    }
    ...
    fn test_tc_4_makefile_has_desktop_release_all_aggregate() {
        ...
        assert!(
            content.contains("desktop-release-windows") && content.contains("desktop-release-linux") && content.contains("desktop-release-macos"),
            "desktop-release-all must reference all three platform tasks"
        );
        assert!(content.contains("run_task"), "desktop-release-all must use run_task for sequential execution");
    }
}
