...
// =============================================================================
//
// `eval_data/` 계열 디렉토리(시나리오/골든셋)의 경로 해석을 한 곳으로 중앙화한다.
//
// 우선순위 (높음 → 낮음):
...
impl DataPaths {
    /// `base` 디렉토리에서 `eval-harness.toml` 을 찾고, 환경변수 + 내장 기본값과
    /// 병합하여 반환한다. 설정 파일이 존재하지 않으면 ENV 와 기본값만 적용한다.
    /// 내장 기본값은 CWD 기준 상대 경로 그대로 둔다.
    ...
    /// desktop 진입점용. `root` 를 기본 검색 위치로 사용하며, 설정 파일도 root 에서
    /// 찾는다. 설정 파일이 없을 때의 내장 기본값도 root 기준으로 join 한다.
...
mod tests {
    ...
    fn write_cfg(dir: &Path, body: &str) {
        fs::write(dir.join(DEFAULT_CONFIG_FILENAME), body).unwrap();
    }
