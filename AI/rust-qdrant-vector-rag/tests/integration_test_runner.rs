...
use common::{
    ...
    TestData, PerformanceMetrics, TestResults, TestResult, verify_test_isolation
};
...
async fn test_document_processing_pipeline() -> TestResult {
    ...
    match setup_test_environment("test_document_processing", 8092).await {
        Ok(container) => {
            ...
            match container.document_service
        ...
        Ok(container) => {
                ...
                .await
            {
                Ok(document_id) => {
                    ...
                    if success_rate >= 0.5 { // Allow 50% success rate for test environment
                        metrics.assert_performance(Duration::from_secs(60));
                        metrics.log_completion();
                        tracing::info!("Question answering pipeline test: PASSED ({}/{} questions)", successful_queries, total_questions);
                        TestResult::Passed
                    } else {
...
async fn test_error_handling_and_edge_cases() -> TestResult {
    ...
    match setup_test_environment("test_error_handling", 8094).await {
        Ok(container) => {
            ...
            match container.document_service.process_document("Just plain text".to_string(), "plain.md".to_string()).await {
                Ok(_) => {
                    tests_passed += 1;
            ...
            if success_rate >= 0.8 { // Require 80% success for error handling
                metrics.log_completion();
...
async fn test_performance_and_scalability() -> TestResult {
    ...
    match setup_test_environment("test_performance", 8095).await {
        Ok(container) => {
            ...
            match container.document_service.process_document(large_content.to_string(), large_filename.to_string()).await {
                Ok(document_id) => {
        ...
        Ok(container) => {
            ...
            for i in 0..3 {
                ...
                let task = tokio::spawn(async move {
                    container_clone.document_service.process_document(content, filename).await
                });
            ...
            for question in &test_data.test_questions[..3] { // Limit to 3 questions
                let container_clone = container.clone();
                let question = question.to_string();
                
                let task = tokio::spawn(async move {
                    container_clone.rag_service.answer_question(question).await
                });
            ...
            if total_operations >= 3 { // Require at least 3 successful operations
                metrics.log_completion();
                tracing::info!("Concurrent operations test: PASSED ({} uploads, {} queries)", successful_uploads, successful_queries);
                TestResult::Passed
            } else {
