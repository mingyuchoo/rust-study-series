...
impl TestServer {
    ...
    async fn simple_health_check(&self) -> Result<serde_json::Value, reqwest::Error> {
        let response = self.client.get(&format!("{}/health/simple", self.base_url)).send().await?;
    ...
    async fn upload_json(&self, content: &str, filename: &str) -> Result<reqwest::Response, reqwest::Error> {
        ...
        self.client.post(&format!("{}/upload/json", self.base_url)).json(&payload).send().await
    }

    async fn upload_multipart(&self, content: &str, filename: &str) -> Result<reqwest::Response, reqwest::Error> {
        let form = multipart::Form::new().text("filename", filename.to_string()).part(
            "file",
        ...
        self.client.post(&format!("{}/upload", self.base_url)).multipart(form).send().await
    }
    ...
    async fn query_json(&self, question: &str) -> Result<reqwest::Response, reqwest::Error> {
        ...
        self.client.post(&format!("{}/query", self.base_url)).json(&payload).send().await
    }
    ...
    async fn query_simple(&self, question: &str) -> Result<reqwest::Response, reqwest::Error> {
        ...
        self.client.get(&format!("{}/query/{}", self.base_url, encoded_question)).send().await
    }
}
            ...
            | Ok(response) => {
                ...
                if response.status().is_success() {
                    match response.json::<RAGResponse>().await {
                        | Ok(rag_response) => {
                            tracing::info!(
                                "Query successful: confidence={:.2}, sources={}",
                                rag_response.confidence,
                                rag_response.source_count()
                            );
            ...
            | Ok(response) => {
                ...
                if response.status().is_success() {
                    match response.json::<RAGResponse>().await {
                        | Ok(rag_response) => {
                            tracing::info!(
                                "Simple query successful: confidence={:.2}, sources={}",
                                rag_response.confidence,
                                rag_response.source_count()
                            );
...
async fn test_api_versioning_and_compatibility() {
    ...
    let v1_endpoints = vec!["/api/v1/health", "/api/v1/upload", "/api/v1/upload/json", "/api/v1/query"];

    for endpoint in v1_endpoints {
        let url = format!("{}{}", server.base_url, endpoint);

        match endpoint {
            | "/api/v1/health" => match server.client.get(&url).send().await {
                | Ok(response) => {
    ...
    let legacy_endpoints = vec!["/health", "/upload", "/query"];

    for endpoint in legacy_endpoints {
        let url = format!("{}{}", server.base_url, endpoint);

        match endpoint {
            | "/health" => match server.client.get(&url).send().await {
                | Ok(response) => {
