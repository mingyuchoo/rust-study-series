...
```bash
rust-sam-app$ sam local invoke HelloWorldFunction --event events/event-name.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
rust-sam-app$ sam local start-api
```

```bash
rust-sam-app$ curl -X GET http://localhost:3000/hello?name=Matthew
```
...
```bash
rust-sam-app$ sam logs -n HelloWorldFunction --stack-name rust-sam-app --tail
```
