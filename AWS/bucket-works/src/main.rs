...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    if arguments.len() != 4 {
        eprintln!(
            "Usage {} <region> <bucket_name> <file_path>",
            arguments[0].clone()
        );
        std::process::exit(1);
    }
    ...
    let region_provider = RegionProviderChain::default_provider()
        .or_else(aws_sdk_s3::config::Region::new(region));
...
async fn create_bucket(
    ...
    let location = BucketLocationConstraint::from_str(region)
        .map_err(|_| format!("Invalid location constraint: {}", region))?;
